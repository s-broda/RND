...
Section~\ref{sec:cubic} replaces those point masses by a cubic spline of \(P\). The inversion remains closed form.
...
\subsection{Cubic Spline of \(P\)}
\label{sec:cubic}

The point-mass measure of Section~\ref{sec:est} puts each increment of \(P\) at one strike. Between knots the distribution function is a step. The estimator interpolates \(P\) by a natural cubic spline through the quoted knots \((K_i,P_i)\). The spline matches value and first derivative at the knots. Its derivative is piecewise quadratic, so the increment of \(P\) is spread across the cell.

Convolution of that derivative with \(\kappa_h\) is closed form. On each piece the integrand is a quadratic times a Gaussian, and the integrals of \(1\), \(u\), and \(u^2\) against \(\phi\) and \(\Phi\) are elementary. Write \(P^{\mathrm{cs}}\) for the spline. The density and the interpolant are
\begin{equation}
\label{eq:cubic}
\hat f_{\Q}(K)
\;=\;
...
\end{equation}
where \(\mathcal{K}_h\) is the Gaussian distribution function of width \(h\). The point-mass sum \eqref{eq:fhat-Q} is the limit of \eqref{eq:cubic} as every cell width tends to zero. These knots are the quoted strikes. Section~\ref{sec:tails} adds the endpoints that complete the distribution.

\subsection{Completing the Tails}
\label{sec:tails}

The spline of Section~\ref{sec:cubic} stops at the first and last quotes. The empirical measure \eqref{eq:nu} puts mass \(P(K_1)\) at the first quoted strike and mass \(1-P(K_m)\) nowhere. Both ends are wrong as a cdf on \((0,\infty)\).
...
\begin{theorem}
\label{thm:ifc}
Let \(\E^{\Q}[S_T]<\infty\), and let \(f_{\Q}\) be four times continuously differentiable on \((0,\infty)\). Let \(C\) be the corresponding call curve. Fix a compact interval \([A,B]\subset(0,\infty)\). Consider a sequence of exact quote grids \(0<K_1^{(m)}<\cdots<K_m^{(m)}\) with mesh \(\delta_m=\max_i(K_i^{(m)}-K_{i-1}^{(m)})\to 0\), \(K_1^{(m)}\le A\) and \(K_m^{(m)}\ge B\). Complete the tails as in Section~\ref{sec:tails} and form the cubic spline of Section~\ref{sec:cubic}. Let \(h_m\to 0\). Then both \eqref{eq:cubic} and its twiced version \eqref{eq:twice} converge to \(f_{\Q}\) uniformly on every compact subset of \((A,B)\).
\end{theorem}
