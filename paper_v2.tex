...
The estimator of this paper is scored twice. The density-scale rule \eqref{eq:h-den} is the rule used on listed chains. A second bandwidth is chosen by even/odd cross-validation of out-of-the-money pricing error. Out-of-the-money premiums are floored at zero before the error is computed. The other methods are:
...
On integrated squared error the ranking is stable across the two targets. The local cubic of the convex projection and the retuned convolution are an order of magnitude closer to \(f_{\Q}\) than the density-scale rule, and the gap is larger for variance gamma, whose density is more peaked. Cross-validating the bandwidth of this paper's estimator on pricing error makes the density worse: the mean integrated squared error rises from \(0.0057\) to \(0.077\) under Heston, and the replication standard deviation from \(0.008\) to \(0.26\). The second-derivative Priestley--Chao bandwidth has integrated squared error comparable to the convolution, and pricing error an order of magnitude larger (\(0.43\) against \(0.045\) under Heston). The same cubic at a pricing bandwidth reverses the two errors. The comparison in which that estimator is run only at \(n^{-1/9}\) and then judged on pricing error is a comparison of bandwidths.

Against the clean calls, SVI has the smallest pricing error on both targets (\(0.029\)). The convolution is next. The density-scale rule is close behind on price (\(0.037\) under Heston, \(0.042\) under variance gamma) and far behind on the density. Figure~\ref{fig:noise-heston} is the median Heston draw: the density-scale curve undershoots the peak and wiggles on the left shoulder; the convolution and SVI track the peak.
...
With one volatility point of noise on an irregular grid, a local cubic of the convex projection and a positive convolution with nodes on the quoted range have smaller integrated squared error than the density-scale rule. A raw SVI slice has the smallest pricing error. Cross-validating this paper's bandwidth on pricing error increases the integrated squared error. On four listed slices the same pattern appears in the prices. A convex projection solved in the call coordinates has in-sample root-mean-square error \(0.022\) on the December S\&P~500 strip. The density-scale interpolant has error \(0.49\). A pricing bandwidth can cut that error and, on this strip, raises the positive mass on the integration window from \(1.01\) to \(42.8\). The second-derivative bandwidth of a kernel of the call, which produces pricing errors above \(20\), is a different estimator from the same kernel at a bandwidth chosen for the call.

The estimator is a fast local approximation of \(f_{\Q}\) when the quotes are exact and the mesh is fine. It is not, on these designs, the most accurate density under noise, and it is not the most accurate interpolant of a listed chain.

\section*{Code and Data Availability}
A Python implementation of \eqref{eq:twice} and the two replication scripts are available at \url{https://github.com/s-broda/RND}. The v2 comparison is \texttt{python/replicate\_v2.py}.
