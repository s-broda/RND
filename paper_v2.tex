...
The two-step row is the same kernel on a different call curve. Under Heston its mean integrated squared error is \(4.3\times 10^{-4}\), below the local polynomial (\(4.7\times 10^{-4}\)), the convolution (\(8.1\times 10^{-4}\)), and SVI (\(2.5\times 10^{-3}\)). Its clean-call error is \(0.029\), the same as SVI to the reported precision. Under variance gamma the integrated squared error is \(2.3\times 10^{-3}\), above the convolution (\(1.7\times 10^{-3}\)) and the local polynomial (\(2.0\times 10^{-3}\)), and the clean-call error is \(0.032\), below both of those and above SVI (\(0.029\)). Figure~\ref{fig:twostep} is the draw that produces the gap: the raw kernel oscillates, and the kernel on the projected call tracks \(f_{\Q}\). The projection contributes no density of its own. Its second derivative is atomic. The kernel is the step that returns \(\hat f_{\Q}\).
...
On these four slices the two-step estimator tracks the raw density-scale kernel. The quotes are already close to the cone, so the projection moves them by hundredths of an index point, and the density-scale bandwidth then does the same smoothing either way. December S\&P~500 error is \(0.50\) against \(0.49\). The unsmoothed projection remains the closest call in the table.
...
Applied to raw noisy quotes, the density-scale rule has larger integrated squared error than a local cubic of the convex projection and than a positive convolution. Applied to the projected call, with the wings held at the wing implied volatility, the same rule has mean integrated squared error \(4.3\times 10^{-4}\) on thirty noisy Heston replications, below both, and clean-call error \(0.029\), level with a raw SVI slice. On noisy variance-gamma quotes the integrated squared error is \(2.3\times 10^{-3}\), a little above the convolution and the local polynomial, and the clean-call error is \(0.032\), below both. The projection's own second derivative is a discrete measure. The kernel is the step that returns a density. On the listed slices the quotes already lie near the cone, the two-step estimator tracks the raw kernel, and the unsmoothed projection remains the closest call: \(0.022\) against \(0.50\) on December S\&P~500. A pricing bandwidth can cut the kernel's error on that strip and raises the positive mass on the integration window from \(1.01\) to \(42.8\).

The estimator is a fast local approximation of \(f_{\Q}\) on a fine exact strip, and, once the calls have been projected, a smooth density that matches the constrained estimators under Heston noise. It is not the closest interpolant of a listed chain that is already nearly convex.
