...
The headline estimator is `python/rnd.py` (`estimate_rnd`). It rescales European calls to a complementary cdf, interpolates that function with a natural cubic spline, completes the unquoted tails, and applies a Gaussian convolution with Schucany–Sommers twicing. The convolution is closed form. `estimate_rnd` chooses the bandwidth internally. The default is \(0.34 F \sigma_{\mathrm{ATM}} \sqrt{T}\, n^{-1/9}\). Passing `h="mesh"` uses 1.2 times the median strike gap.
